...
class GradingController extends Controller
{
    public function gradeStudent(Request $request, Course $course, User $student)
    {
        ...
        $currentStatus = $enrolled->pivot->pass_status ?? 'pending';
        if ($currentStatus !== 'pending') {
            if ($request->wantsJson() || $request->ajax()) {
                return response()->json(['message' => 'Student has already been graded.'], 422);
            }

            return back()->withErrors(['error' => 'Student has already been graded.']);
        }
        ...
        $pivotData = ['pass_status' => $validated['pass_status']];
